...
//                      and ra.employee.id = :assignedEmployeeUserId
//                )
//              )
...
//            @Param("assignedEmployeeUserId") Long assignedEmployeeUserId,
...
//    );
    ...
    @Query("""
        ...
        left join AiSuggestion ai on ai.request = r
        left join ai.category aiCategory
        left join aiCategory.department aiDepartment
        ...
        where (:id is null or r.id = :id)
          ...
          and (
                ...
                or (
                      ai.id is not null
                      and ai.suggestionStatus = :pendingSuggestionStatus
                      and aiDepartment.id = :departmentId
                   )
                or (
                      (ai.id is null or ai.suggestionStatus <> :pendingSuggestionStatus)
    ...
    Page<RequestPageableProjection> findFiltered(
            ...
            @Param("routingStatus") RoutingStatus routingStatus,
            @Param("priority") Priority priority,
            @Param("pendingSuggestionStatus") SuggestionStatus pendingSuggestionStatus,
            @Param("text") String text,
            @Param("submittedFrom") LocalDateTime submittedFrom,
            @Param("submittedTo") LocalDateTime submittedTo,
            Pageable pageable
